...
import static org.athento.nuxeo.rm.api.utils.ArchivoConstantes.NUMERO_UNO;
	...
	private List<LogEntry> computeLogEntries(DocumentModel document) {
		...
		if (document != null) {
			try {
				...
				StringBuffer query = new StringBuffer().append("log.docUUID like ")
						.append(document.getId());
				query = query.
						append("order by log.eventDate desc");
				logEntries = logsBean.nativeQueryLogs(query.toString(), NUMERO_UNO,
						Integer.MAX_VALUE);
			} catch (Exception e) {
